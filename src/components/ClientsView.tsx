      ...
      <div className="hidden md:block bg-white border-b border-slate-200/60 p-4 shrink-0 shadow-xxs overflow-x-auto no-scrollbar">
        <div className="flex gap-4 min-w-[700px]">
          {[
            ...
            { label: 'Active Clients', count: activeClientsCount, desc: 'With live trials', icon: Activity, border: '#10b981', bg: '#f0fdf4', badge: 'Active', click: () => { setFilterTab('ACTIVE'); setSelectedMetric('Active Clients'); } },
            { label: 'New This Month', count: newThisMonthCount, desc: 'Recent intakes verified', icon: User, border: '#8b5cf6', bg: '#f5f3ff', badge: 'Monthly', click: () => { setFilterTab('ALL'); setSearchQuery(''); setSelectedMetric('New This Month'); } },
            { label: 'Pending Onboarding', count: pendingOnboardingCount, desc: 'Incomplete checklists', icon: Clock, border: '#f59e0b', bg: '#fffbeb', badge: 'Onboard', click: () => { setFilterTab('ONBOARDING'); setSelectedMetric('Pending Onboarding'); } },
            { label: 'Follow-ups Due', count: followUpDueCount, desc: 'Overdue task queues', icon: AlertTriangle, border: '#ef4444', bg: '#fef2f2', badge: 'Due', click: () => { setFilterTab('ALL'); setSelectedMetric('Follow-ups Due'); } },
            { label: 'Inactive Clients', count: inactiveClientsCount, desc: 'No cases currently', icon: ShieldAlert, border: '#64748b', bg: '#f1f5f9', badge: 'Inactive', click: () => { setFilterTab('NO_MATTERS'); setSelectedMetric('Inactive Clients'); } }
          ].map(card => {
            ...
            return (
              <button
                key={card.label}
                onClick={card.click}
                className={`top-stat-card flex flex-col justify-between cursor-pointer transition-all duration-200 select-none ${
                  isActive 
              ...
              >
                <div className="flex items-center justify-between w-full">
                  <Icon className="h-4.5 w-4.5 shrink-0" style={{ color: card.border }} />
